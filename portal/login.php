<?php
declare(strict_types=1);

require_once __DIR__ . '/../includes/db.php';
require_once __DIR__ . '/../includes/auth-portal.php';

portal_session_start();

$token = (string) ($_GET['token'] ?? '');
$error = '';

if ($token === '') {
    $error = 'Este link no es válido.';
} else {
    $pdo  = db();
    $hash = hash('sha256', $token);

    /* UPDATE atómico primero: si dos requests llegan con el mismo token en
       simultáneo, solo uno va a afectar una fila (evita reuso por carrera). */
    $update = $pdo->prepare(
        'UPDATE magic_link_tokens
         SET used_at = NOW()
         WHERE token_hash = ? AND used_at IS NULL AND expires_at > NOW()'
    );
    $update->execute([$hash]);

    if ($update->rowCount() === 1) {
        $stmt = $pdo->prepare('SELECT client_id FROM magic_link_tokens WHERE token_hash = ?');
        $stmt->execute([$hash]);
        $row = $stmt->fetch();

        portal_login((int) $row['client_id']);
        header('Location: /portal/dashboard.php');
        exit;
    }

    $error = 'Este link ya venció o ya fue usado. Pedí uno nuevo.';
}

$pageTitle = 'Ingresar';
require __DIR__ . '/inc/header.php';
?>
<main class="container portal-auth">
  <p class="portal-alert"><?= htmlspecialchars($error, ENT_QUOTES, 'UTF-8') ?></p>
  <a href="/portal/index.php" class="btn btn--ghost">Pedir un nuevo link</a>
</main>
<?php require __DIR__ . '/inc/footer.php'; ?>
